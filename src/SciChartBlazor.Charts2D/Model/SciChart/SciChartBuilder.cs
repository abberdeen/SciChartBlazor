...
namespace SciChartBlazor.Charts2D.Model.SciChart;
...
public class SciChartBuilder : IDisposable
{
    ...
    public async Task AddSeries(IList<RenderableSeriesBase> series)
    {
        foreach (var single in series)
        {
            ...
            {
                series[i].Id = ids[i];
                _renderableSeries.Add(series[i]);
            }
        }
    }
    /// <summary>
    ...
    public async Task AddAnnotations(IList<AnnotationBase> annotations)
    {
        ...
        for (int i = 0; i < annotations.Count(); i++)
        {
            ...
            _annotations.Add(annotations[i]);
        }
    }
    /// <summary>
    /// Add an annotation.
    ...
    public async Task RemoveRenderableSeries(string renderableSeriesId)
    {
        ...
        _renderableSeries.Remove(_renderableSeries.First(x => x.Id == renderableSeriesId));
    }
